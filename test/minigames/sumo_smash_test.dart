...
void main() {
  ...
  test('PACING: all-bot 4p round lasts > 1.5s and finishes within the limit',
    ...
    for (final seed in const [1, 7, 13, 21, 99]) {
      ...
      expect(seconds, greaterThan(1.5),
  ...
  group('StarController (chaos pickup)', () {
    ...
    test('spawns only after the first-spawn delay and only with >= 2 alive', () {
      final c = build();
      ...
      expect((c.star!.pos - center).distance, lessThanOrEqualTo(300 * 0.4 + 1));
    });
